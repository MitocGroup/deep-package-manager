...
suite('Provisioning/Service/LambdaService', () => {
  ...
  test('Check AVAILABLE_REGIONS() static method returns array of available regions', () => {
    ...
    chai.expect(LambdaService.AVAILABLE_REGIONS).to.be.include(Core.AWS.Region.EU_FRANKFURT);
    chai.expect(LambdaService.AVAILABLE_REGIONS).to.be.include(Core.AWS.Region.ASIA_PACIFIC_TOKYO);
    chai.expect(LambdaService.AVAILABLE_REGIONS).to.be.include(Core.AWS.Region.ASIA_PACIFIC_SEOUL);
  });
